...
const LoginPage = () => {
  ...
  const onSubmit = (data: FormData) => {
    ...
    auth.login({ matricula, password, rememberMe }, () => {
      setError('matricula', {
        type: 'manual',
        message: 'Matricula ou Senha está inválido!'
      })
    })
  }
  ...
  return (
    <Box className='content-right'>
      {!hidden ? (
        <Box sx={{ p: 12, flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}></Box>
      ) : null}
      <RightWrapper
      ...
      >
        <Box sx={{ mx: 'auto', maxWidth: 400 }}>
          ...
          <Typography variant='h6' sx={{ mb: 1.5 }}>
            Seja bem-vindo ao {themeConfig.templateName}! 👋🏻
          </Typography>
          <Typography sx={{ mb: 6, color: 'text.secondary' }}>
            Faça login com suas credenciais para iniciar sua sessão!
          </Typography>
          <form noValidate autoComplete='off' onSubmit={handleSubmit(onSubmit)}>
            <FormControl fullWidth sx={{ mb: 4 }}>
              ...
              />
              {errors.matricula && (
                <FormHelperText sx={{ color: 'error.main' }}>{errors.matricula.message}</FormHelperText>
              )}
            </FormControl>
            ...
            >
              ...
              />
              <LinkStyled href='/faq'>Precisa de Ajuda?</LinkStyled>
            </Box>
