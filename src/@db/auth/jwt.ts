...
mock.onPost('/jwt/login').reply(request => {
  const { matricula, password } = JSON.parse(request.data)

  let error = {
    matricula: ['Something went wrong']
  }
  ...
  if (user) {
    ...
    error = {
      matricula: ['Matricula ou Senha inválido!']
    }
